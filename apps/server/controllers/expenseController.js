...
exports.createExpense = async (req, res) => {
    ...
    try {
        ...
        if(split_type!='none') {
            if(split_members.length===0){
                return res.status(400).json({ msg: 'Split members array cannot be empty' });
            }}
        ...
        if (split_type === 'equal') {
            ...
            processedSplitMembers = split_members.map(({user}) => ({ user, amount: splitAmount }));
